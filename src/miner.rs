use crate::{
    pow,
    proto::{CryptixdMessage, RpcBlock},
    swap_rust::WatchSwap,
...
pub struct MinerManager {
    ...
    send_channel: Sender<CryptixdMessage>,
...
impl MinerManager {
    ...
    pub fn launch_cpu_miner(
        ...
        #[cold]
        fn found_block(send_channel: &Sender<CryptixdMessage>, block: RpcBlock) -> Result<(), Error> {
            let block_hash = block.block_hash().expect("We just got it from the state, we should be able to hash it");
            send_channel.blocking_send(CryptixdMessage::submit_block(block))?;
    ...
    async fn log_hashrate(hashes_tried: Arc<AtomicU64>) {
        ...
        for i in 0u64.. {
            ...
            if hashes == 0 && i % 2 == 0 {
                warn!("Cryptixd is still not synced");
            } else if hashes != 0 {
